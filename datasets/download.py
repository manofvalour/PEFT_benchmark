...
mask loss on prompt token, and train_test_split
"""
...
def raw_text_chunker(text: str, chunk_size: int = 1024, overlap: int = 128) -> list[str]:
...
class DataBuilder:
    ...
    def build_med_qa_dataset(source_path: str, output_path: str):
        """
        Example: med data to instruction format.
        Input JSON: [{"question": ..., "answer": ..., "context": ...}]
        """
        with Path.open(source_path) as f:
            data = json.load(f)
        ...
        logger.info(f"Saved {len(ds)} Data QA samples to {output_path}")
        return ds
    ...
    def build_legal_dataset(source_path: str, output_path: str):
        ...
        """
        with Path.open(source_path) as f:
            data = json.load(f)  ## loading the dataset
    ...
    def load_csv_as_instruct(
        ...
        """Generating instruction format dataset from CSV file
            csv_path: path to the csv raw file
            output_path: path to save the processed file
            instruction_col: Name of the column that contain the context
            output_col: Name of the column that contain the response
            input_col: Name of the column that contain the user input
            system_prefix: the system instruction for the content
        Returns:
            Dataset : Returns the dataset in instruction format
        """

        rows = []
        with Path.open(csv_path, newline="", encoding="utf-8") as f:
            csv_reader = csv.DictReader(f)  ##reading the csv dataset
...
def custom_data_collator(tokenizer, response_template: str):
    """
